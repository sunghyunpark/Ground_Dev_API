...
router.post('/view/comment', function(req, res){
  ...
  var sql = 'SELECT a.fcm_token, b.area_no, b.board_type FROM users AS a JOIN '+tableNameOfCommnetArticle+' AS b ON(a.uid = b.writer_id) WHERE b.no=?';
  conn.query(sql, [articleNo], function(err, result, fields){
    if(err){
      console.log(err);
    }else{
      fcmModule.sendPushMyArticleByComment(result[0].fcm_token, result[0].area_no, result[0].board_type);
      console.log('push ok');
    }
  })
